...
| :---- | :---- | :---- |
| 1 | Quyền riêng tư & bảo vệ dữ liệu trẻ em ở mức cao nhất | Tuyệt đối KHÔNG giám sát nội dung riêng tư hay tin nhắn cá nhân của sinh viên. Chỉ dùng dữ liệu vận hành có sẵn ở dạng metadata. |
...
## **D.5 OUTPUT — Phân loại sinh viên (2 trục rạch ròi)**

Output tách thành **hai trục độc lập nhưng có tương quan**. Việc tách trục là bắt buộc: một sinh viên có thể có nguy cơ học vụ cao mà không có dấu hiệu wellbeing, và ngược lại. Gộp chung sẽ che mất bản chất và dẫn đến can thiệp sai loại.

### **D.5.1 Trục 1 — Nguy cơ bỏ học (Dropout Risk)**
...
## **D.6 Luồng logic hệ thống (cho Backend / Dev)**

Pipeline được mô tả theo tầng để nhóm backend triển khai chuẩn chỉ. Mỗi tầng có input/output rõ ràng và ràng buộc riêng.

| Tầng | Chức năng | Input → Output | Ràng buộc bắt buộc |
| :---- | :---- | :---- | :---- |
| L1 Ingestion | Thu thập metadata từ SIS, LMS, điểm danh, điểm quá trình. | Nguồn vận hành → bản ghi metadata pseudonymized. | Chỉ metadata; loại bỏ nội dung; gắn nguồn & timestamp. |
| L2 Feature | Tính đặc trưng within-student & so cohort ẩn danh; gắn coverage score. | Metadata → feature vector \+ coverage. | Loại biến kinh tế/dân tộc & các proxy của chúng. |
| L3 Scoring | Hợp nhất đa tín hiệu → (Dropout Risk, Wellbeing Check) kèm uncertainty. | Feature → 2 điểm \+ độ tin cậy. | Multi-signal cho trục W; calibrate để kiểm soát false-alarm. |
| L4 Fairness gate | Audit disparate impact theo nhóm; chặn output nếu lệch bất thường. | Điểm → điểm đã qua kiểm định. | Không cảnh báo khi coverage thấp; log audit. |
| L5 Presentation | Hiển thị theo tầng quyền; danh sách ưu tiên \+ lý do tổng hợp. | Điểm → dashboard theo vai trò. | Ẩn breakdown wellbeing với T1; TTL cho lý do. |
| L6 Handoff | Định tuyến escalation sang con người; ghi nhận phản hồi. | Cảnh báo → ca được chuyển giao. | Không hành động tự động; luôn có con người quyết định. |

### **D.6.1 Quy trình chuyển giao con người (Handoff) — tiêu chí chấm điểm trọng yếu**

1. Agent đưa sinh viên vào **danh sách ưu tiên** của đúng người phụ trách (theo phân tầng quyền), kèm lý do tổng hợp và mức tin cậy.

2. Người phụ trách **quyết định** có tiếp cận hay không — agent không tự động làm gì.

3. Với ca D1,D2 cuộc tiếp cận là **warm check-in** (hỏi thăm, đề nghị giúp đỡ) — không dò hỏi chẩn đoán, không kỷ luật.

4. Ca D3 hoặc vượt ngưỡng crisis được **chuyển tuyến chuyên trách** (công tác sinh viên). CVHT không phải điểm cuối.

5. Kết quả tiếp cận được ghi nhận (đã hỗ trợ / false-alarm) làm **feedback** để hiệu chỉnh ngưỡng và giảm false-alarm theo thời gian.

**Nguyên tắc “human-in-the-loop”:** mọi cảnh báo là **decision support**, không phải **decision**. Con người luôn là người ra quyết định cuối cùng và thực hiện chăm sóc.
