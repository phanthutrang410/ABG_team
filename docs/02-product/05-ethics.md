...
MVP chỉ chạy trên dữ liệu synthetic. Bất kỳ pilot nào với dữ liệu thật đều cần một vòng phê duyệt riêng về mục đích, căn cứ sử dụng, thông báo, phân quyền, lưu giữ, sửa sai và xử lý sự cố theo chính sách nhà trường và pháp luật áp dụng.

## 2. Privacy by design

### Được phép trong MVP

- Điểm theo thời gian và feature xu hướng/biến động được suy ra từ điểm.
- Tỷ lệ chuyên cần theo thời gian và feature xu hướng được suy ra từ chuyên cần.
- Mã sinh viên synthetic và mapping lớp/cohort cần cho demo bàn giao.
- Thuộc tính nhóm **synthetic** chỉ để tính fairness; không đưa vào model scoring và không giải thích case cá nhân.
...
- PII sinh viên thật trong repo, log, ảnh chụp màn hình, slide hoặc video demo.
...
- **Retention:** MVP chỉ dùng synthetic; pilot thật phải có thời hạn lưu theo từng loại dữ liệu và xóa tín hiệu/case khi hết mục đích.
...
| Admin kỹ thuật/Data-ML | Dữ liệu synthetic hoặc pseudonymized cần cho vận hành/kiểm định, log kỹ thuật | Danh tính thật gắn với score; không dùng tài khoản kỹ thuật để duyệt hay hỗ trợ case |
...
| Threshold transparency | Hiển thị tác động của ngưỡng tới precision/recall, FPR và số case cần review trên synthetic |
...
- Chỉ đo trên thuộc tính nhóm synthetic và ghi rõ điều đó cạnh mọi metric.
...
Ngưỡng nội bộ dùng cho demo phải được ghi cùng dataset/model version; không chuyển nguyên sang dữ liệu thật.
...
- [ ] Model được validate/calibrate trên bối cảnh trường mục tiêu; không dùng kết quả synthetic để suy rộng.
