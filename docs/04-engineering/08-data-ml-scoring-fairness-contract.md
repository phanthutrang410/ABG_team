...
>
> Contract nguồn chuẩn cho scoring, coverage, threshold, evaluation và fairness trên **snapshot EPU đã qua gate** — **không** synthetic. Schema Pydantic (`H06a`/`H06c`) phải mã hóa đúng tài liệu này. Khi lệch, sửa schema hoặc mở decision — không âm thầm chọn một bản.
>
> **Phân biệt gate:** `M05a` (code gate) Done ≠ dữ liệu đã duyệt (`M05b`). Thiếu approval → zero import / `insufficient_data`.
...
|:---|:---|:---|
| `dataset_version` | Định danh snapshot đã duyệt: `source_id` + hash ngắn + schema_version | Chỉ từ `source_manifest` sau M05b; **cấm** `synthetic-*` trên MVP path |
...
| Feature | Định nghĩa | Điều kiện hợp lệ |
|:--|:--|:--|
| `attendance_rate_window` | Tỷ lệ `presence_status=present` trong cửa sổ | Sau `H15` approval; ≥ **4** mốc `observed_at` có `presence_status` khác null |
| `attendance_trend_slope` | Độ dốc theo `observed_at` | ≥ **2** mốc phân biệt sau khi rate gate đạt |

**Default cửa sổ quan sát (H10 → H15 được refine khi có nguồn):** 90 ngày lịch gần nhất tính từ `status_observed_at`/`extracted_at` của snapshot, hoặc đúng học kỳ đang mở nếu export định nghĩa period — ưu tiên định nghĩa period của data owner khi có.

**Cho tới khi `H15` có approval artifact:** toàn nhánh trả `insufficient_data` với `reason_code=attendance_source_unapproved`. Cấm impute 0, tạo chuỗi tuần, dùng field `Vắng CP/KP` snapshot synthetic.

`excused` / nghỉ có phép: theo exception policy trong amendment `H15`; trước đó không suy diễn.
...
| `attendance_source_unapproved` | Chưa có approval `H15` |
...
| Lớp | Kiểm tra | Fail ⇒ |
|:--|:--|:--|
| Register | Allowlist (v59-empty primary; epu_data = regression transform); từ chối synthetic theo tên + nội dung | Từ chối nguồn |
