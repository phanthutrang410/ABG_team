...
| `ReviewCase` safe projection | `review_priority_band`, factors/evidence, coverage, freshness, data state, limitations, model version/thời điểm tính, case state | UI và Agent context sau scope filter | H06a + H11a envelopes Done; **không** đồng nhất với `TransitionResponse` H06b; HTTP list/detail vẫn H02 |
| Care state/history | State chuẩn, action của con người/hệ thống, actor, timestamp, reason/review time, `mapping_repair_queued` | Workflow API, reviewer/recipient theo scope | H06b in-memory core; **H03 Done** — assign resolves `advisor_ref` via H08 (client ignored); persistence production vẫn sau |
