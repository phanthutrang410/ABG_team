...
> **Status:** Research lock (decision #20) · Owner research: Hoàng (`H21`) · Build: `H22` / `G06` (stretch sau critical path G02/D4b).
...
### 4.2 Public envelope (đề xuất `H22`)

```text
AdvisorHandoffDraftBundle
  advisor_ref: string          # pseudonym routing only
  case_count: int
  cases[]:
    case_id, student_ref, review_priority_band,
    contributing_factor_codes[], coverage/freshness summary,
    case_state
  draft:
    subject: string            # trung lập, không “nguy cơ bỏ học”
    body: string               # danh sách student_ref (+ lớp nếu public DTO cho phép)
    requires_human_approval: true  # invariant
  limitations[]: string        # H12a keys / insufficient contact map
```

**Cấm** trong envelope public / agent / FE fixture: email/SĐT/họ tên GV hoặc SV, MSSV, `model_score`, `is_dropout_outcome`, audit-group attrs, raw DWH.
...
|:--|:--|:--|:--|
| `H21` | Research + contract này khóa; không build | — | Doc khớp Ethics/Process; decision #20 |
| `H22` | API bundle theo `advisor_ref` + fixture/test | H21, H03, H02 | Happy + mapping_repair + forbidden-field scan |
| `G06` | FE filter/group + Copy/`mailto:` draft | H22, G05 | Lint/build; không PII; không auto-send |
